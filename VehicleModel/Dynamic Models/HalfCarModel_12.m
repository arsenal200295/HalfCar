function [Zdot] = HalfCarModel_12(t,z,hcar)
%HALFCARMODEL4 ITERATION 7 -  Half Car Model to be called by the Numerical Integrator
%   ** NEW - CONTROLLER USED FOR CONVERSION OF ACCELERATION TO
%   DRIVING/BRAKING TORQUE
%   ACCELERATION INPUT CONVERTED TO DRIVING/BRAKING TORQUE
...
Fx_In_2 = Fx_In * (1 - hcar.simInputs3.forceDis);

% --------------------- ROAD INPUT ------------------------ %
% Computing delay of the input signal to reach rear axle
delay = hcar.w2w/Vx;
zr1 = interp1(hcar.simInputs3.time,hcar.simInputs3.zr1,t+delay,'linear','extrap')*1;
zr2 = interp1(hcar.simInputs3.time,hcar.simInputs3.zr2,t,'linear','extrap')*1;
...
% --------------------- DRIVING TORQUE ------------------------ %
Md_1 = (Fx_In_1 + (Fx_In_1 - fxy_1))*(rL1)*hcar.simInputs3.GainMD_1;
Md_2 = (Fx_In_2 + (Fx_In_2 - fxy_2))*(rL2)*hcar.simInputs3.GainMD_2;
