function [Zdot] = HalfCarModel7(t,z,hcar)
...
fz1_s = -fxy_1*tan(th1);% + (torqueWheel1/hcar.axleSusp.vsal_1);% Reaction component on Sprung Mass
fz2_s = fxy_2*tan(th2);% - (torqueWheel2/hcar.axleSusp.vsal_2);% Reaction component on Sprung Mass
...
fnsm = (((ma1 + ma2))*acc*hcar.cg_USM_Height)/hcar.w2w;
...
phi_dd = (-hcar.dis_a12cg*fs1 - hcar.dis_a12cg*fd1...
         ...
         - (fxy_1 + fxy_2)*(hcar.cg_SM_Height + (zs-init(1)))...
         - (fz1_s*hcar.dis_a12cg) + (fz2_s*hcar.dis_a22cg) )/Is;

    th1_dd = (torqueWheel1 - fnetv1*hyp1*sin(pi/2 + th1)- fxy_1*hyp1*sin(th1))/...
             (Iapc1);

za1_dd = (-fs1 - fd1 - fga1 + fkt1 + fdt1 + fnsm + fz1_u)/hcar.mass_usm_1 - ...
          th1_dd*(hyp1);

    th2_dd = (torqueWheel2 + fnetv2*hyp2*sin(pi/2 + th2)- fxy_2*hyp2*sin(th2))/...
             (Iapc2);
