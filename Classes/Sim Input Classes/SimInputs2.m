classdef SimInputs2
    ...
    %   ** NEW - DRIVING/BRAKING TORQUE INPUTS
...
%             [~,~,def_Zin] = gauspuls(obj.time-15,0.2,0.6);
            ...
            % Creating a default driving torque signal
            obj.Md_1 = obj.Create_RectPulse(obj.time,-1000,5,10);
            obj.Md_2 = obj.Create_RectPulse(obj.time,-1000,5,10);
%             obj.Md_1 = obj.Create_Step(obj.time,5,-2000);
%             obj.Md_2 = obj.Create_Step(obj.time,5,-2000);
        end 
    end
    ...
    methods (Access = public) 
        ...
        function [fxy_1,fxy_2] = Compute_ContactPatchForces(obj,hcar)
            ...
            ftot = m.*obj.acc*9.81;
            fxy_1 = (obj.forceDis/100).*ftot;
            fxy_2 = (1-(obj.forceDis/100)).*ftot;
        end
    end
